...
| M4 | HIGH | First run (imgsz 1280) failed two gates for dataset reasons, not model reasons: 5 val/test frames were mid-animation transients (cards captured mid-slide/mid-flip render distorted sprites — template GT missed 4 real cards, and the one sub-threshold King was a mid-hole-reveal render). Fix: cross-model disagreement audit (`scripts/audit_labels.py`, the systematized M3 hand-verify step) + human-verified per-session `exclude.json` consumed by `vision/dataset.py`. On curated GT at 1280: mAP50 1.000, per-rank recall 1.000 |
| M4 | HIGH | 1280-input CPU latency is 249 ms (gate <100 ms), and a smaller EXPORT of a 1280-trained model collapses recall (960 export: rank-2 recall 0.40) → run 2 retrains AT the deployment input size (imgsz 640; ~55 ms measured locally on this CPU), exporting 640 + an 800 fallback. Detector input size now auto-detected from the ONNX static shape so a weights swap can never silently mismatch the letterbox |
| M4 | MED | Latency gate is timed on a separately-constructed detector at the DEPLOYED confidence — at the 0.001 AP-sweep threshold, pure-Python NMS over background anchors fakes a latency failure (review finding). Sub-threshold domain-gap FP observed and accepted: the yellow "Repeat Bet & Deal" button reads as Tc at conf ~0.5 (synthetic backgrounds carry no UI chrome); revisit only if it ever clears the 0.80 runtime threshold |
| M4 | LOW | Capture sessions to date were played at the trainer's 8-deck default — irrelevant for detection data (identical card art), but Phase 8 eval sessions MUST set 6 decks / 75-80% penetration (PRD reference config) |
| M6 | MED | Region selection reuses the proven `find_table` auto-location (+ count-bar extension, persisted to gitignored config.json; `--relocate`/`--region` overrides) instead of the sketched drag-once translucent-window UI — same §4.3 self-calibration makes precision immaterial; the drag UI is deferred unless auto-location proves unreliable in the M6 smoke test |
| M6 | LOW | Worker surfaces exceptions to the overlay as warnings and keeps running (a dead worker would silently freeze the count); hotkeys are `<ctrl>+<alt>+c/r/q` (capture/reset/quit), configurable in config.json |
